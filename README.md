...
The **[threat model](docs/threat-model.md)** is written for a parser, which
has exactly one adversary: **whoever controls the bytes**. They choose every
byte, the length, the nesting depth, and where the input is cut off.

Two things it says plainly:

- **A panic here is a halt.** In `no_std` there is nothing to catch one, so
  robustness against crafted input is a security property, not a quality one.
  The evidence is ten property tests over every truncation and every
  single-byte corruption of a real corpus — not a handful of examples.
- **There is no continuous fuzzing yet.** The suite is bounded and
  reproducible, which a fuzzer is not, and it is a floor rather than a
  substitute. That is written down as a residual risk with the condition that
  closes it.

<!-- HARDENING-TABLE:BEGIN generated by use-protection-please — edit docs/plans/use-protection-please.md, not this block -->
## Hardening status

**Tier** critical-path · **Audited** 2026-09-21 (survey) · **v1.0.0 gates** 10/16 · [Full checklist](docs/plans/use-protection-please.md)
